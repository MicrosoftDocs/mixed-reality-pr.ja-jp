...
HoloLens には、reprojection と呼ばれる高度なハードウェア支援型の holographic 安定化手法があります。 シーンがアニメーション化され、ユーザーが頭を動かしたときに、再プロジェクションでは、視点 (CameraPose) の動きと変化が考慮されます。  アプリケーションでは、reprojection を最大限に活用するために、特定のアクションを実行する必要があります。
