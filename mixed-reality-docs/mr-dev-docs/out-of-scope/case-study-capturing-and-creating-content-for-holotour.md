...
HoloLens で [空間サウンド](../design/spatial-sound.md) 機能を利用することは、convincingly の実際のイマーシブエクスペリエンスを作成するうえで重要です。 三脚のカメラの下にある4つのマイクの配列を使用しました。これにより、カメラの位置から4つの方向にサウンドがキャプチャされます。これにより、バックグラウンドで空間サウンドを作成するための十分な情報が得られます。
